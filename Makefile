...
debug:
	cargo install --path .
	RUST_LOG=scale::sui=debug scale -l /Users/lihua/work/lihua/blok/scale/scale/sui.log  bot -b sui -p 8081 -t 2
sub:
	cargo install --path .
	RUST_LOG=scale::http::service=debug scale  bot -b sui -p 8081 -t 2
config:
